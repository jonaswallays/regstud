...
if ($_POST) {
    ...
    $attach = "1314_KASKCONS_STUDIEWIJZER_LR.pdf";
    //einde ART
    if (isset($result)) {
        $msg = $fname . " " . $lname . " is succesvol toegevoegd.";
        success($msg, $email, $fname, $lname, $attach);
    } else {
...
function db_connection($query) {
    ...
    /* $dbhost = 'localhost';
      ...
      $dbpass = 'student';
      $dbname = 'fbo_data'; */
    mysql_connect($dbhost, $dbuser, $dbpass) OR die(fail('Could not connect to database.'));
...
function success($message, $email, $fname, $lname, $attach) {

    mailNow($attach, $email, $fname . ' ' . $lname);
    //die(json_encode(array('status' => 'success', 'message' => $message)));
}

function mailNow($attach, $towho, $toname) {
    date_default_timezone_set('Europe/Brussels');
    require_once('PHPMailer/class.phpmailer.php');

    $mail = new PHPMailer(); // defaults to using php "mail()"
    //$mail->IsSendmail();             // enables SMTP debug information (for testing)
    $body = file_get_contents('contents.html');
    $body = eregi_replace("[\]", '', $body);

    $mail->IsSMTP(); // telling the class to use SMTP
    $mail->Host = "mail-out.hogent.be"; // SMTP server
    //$mail->SMTPDebug = 2;

    //$mail->AddReplyTo("name@yourdomain.com", "First Last");

    $mail->SetFrom('info@hogent.be', 'Info Hogeschool Gent');
    ...
    $mail->Subject = "Aangevraagde informatie HoGent op SID";

    //$mail->AltBody = "To view the message, please use an HTML compatible email viewer!"; // optional, comment out and test

    $mail->MsgHTML($body);

    $mail->AddAttachment("pdf/" . $attach);      // attachment
    //$mail->AddAttachment("images/phpmailer_mini.gif"); // attachment


    if (!$mail->Send()) {
        die(json_encode(array('status' => 'success', 'message' => "Mail not sent")));
    } else {
        die(json_encode(array('status' => 'success', 'message' => $toname . " is succesvol toegevoegd. E-mail verstuurd.")));
    }
}
